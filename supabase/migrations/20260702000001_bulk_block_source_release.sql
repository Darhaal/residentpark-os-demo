-- Title: Bulk Block — Release Source Spot Before Relocation
-- Path: supabase/migrations/20260702000001_bulk_block_source_release.sql
-- Functionality: Fixes tx_bulk_block_and_relocate: the relocation loop occupied the
--   target spot while the source spot still carried the same assigned_vehicle_id, so
--   the one-spot-per-vehicle unique index (unique_active_vehicle_spot) rejected every
--   relocation with 23505 (caught by parking-bulk-block.db.test.ts on CI run #49).
--   The loop now clears the source spot's denormalized assignment BEFORE occupying the
--   target — the same order tx_transfer_parking_spot uses. No other behavior change.
...
BEGIN
  ...
  LOOP
    ...
    IF FOUND THEN
      ...
      -- Release the source spot's denormalized assignment BEFORE occupying the
      -- target: unique_active_vehicle_spot allows one spot per vehicle, so copying
      -- the vehicle to the target while the source still holds it raises 23505.
      -- (The final bulk UPDATE blocks and re-clears these spots afterwards.)
      UPDATE public.parking_spots
      SET assigned_apartment_id = NULL,
