-- Title: Resident Parking Map (de-ambiguated)
-- Path: supabase/migrations/20260628000001_resident_parking_map.sql
-- Functionality: Brings the formal chain's get_resident_parking_map() up to the version
--   already applied to live (supabase/applied/get_resident_parking_map.sql). The baseline
--   definition references a bare `id` in the caller-lookup, which is ambiguous against the
--   RETURNS TABLE output column `id` ("column reference \"id\" is ambiguous") and leaves the
--   resident map unavailable in any DB rebuilt from the chain. This forward migration
--   replaces it with the table-aliased version. CREATE OR REPLACE + GRANT, idempotent.
